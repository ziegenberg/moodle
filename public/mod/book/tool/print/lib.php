...
function booktool_print_extend_settings_navigation(settings_navigation $settings, navigation_node $node) {
    ...
    if (has_capability('booktool/print:print', $settings->get_page()->cm->context)) {
        ...
        $action1 = new action_link($url1, get_string('printbook', 'booktool_print'), new popup_action('click', $url1));
        $text1 = get_string('printbook', 'booktool_print');
        $text1 += html_writer::span('(' . get_string('opensinnewwindow') . ')', 'visually-hidden');
        ...
        $action2 = new action_link($url2, get_string('printchapter', 'booktool_print'), new popup_action('click', $url2));
        $text2 = get_string('printchapter', 'booktool_print');
        $text2 += html_writer::span('(' . get_string('opensinnewwindow') . ')', 'visually-hidden');
