...
```sh
mv lib/symfony/filesystem/readme_moodle.txt ./
rm -rf lib/symfony/filesystem
installdir=`mktemp -d`
cd "${installdir}"
...
cd -
cp -rf "${installdir}/vendor/symfony/"* lib/symfony/
mv readme_moodle.txt lib/symfony/filesystem
rm -rf $installdir
git add .
```
