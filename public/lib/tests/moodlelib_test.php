...
final class moodlelib_test extends \advanced_testcase {
    ...
    public function test_address_in_subnet(): void {
        ...
        // A partial prefix of more than one group that ends in a trailing colon (e.g. 'baba:baba:'
        // or '2001:db8:') is a valid IPv6 subnet prefix. The trailing colon is trimmed and the
        // remaining groups are zero-padded into a full subnet mask, so the prefix must match its
        // own range but no other.
        $this->assertTrue(address_in_subnet('baba:baba::bab', 'baba:baba:'));
        $this->assertFalse(address_in_subnet('baba::bab', 'baba:baba:'));
        $this->assertTrue(address_in_subnet('2001:db8::1', '2001:db8:'));
        $this->assertFalse(address_in_subnet('2001:db9::1', '2001:db8:'));

        // IPv6 addresses ending in '::' must match (and be matched by) their exact address, just like
        // their uncompressed and otherwise compressed equivalents.
